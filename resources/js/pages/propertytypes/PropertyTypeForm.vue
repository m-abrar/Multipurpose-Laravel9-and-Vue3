...
const createPropertyType = (values, actions) => {
    axios.post('/api/propertytypes/create', form)
        .then((response) => {
            router.push('/admin/propertytypes');
            toastr.success('Property type created successfully!');
        })
        .catch((error) => {
...
const editPropertyType = (values, actions) => {
    axios.put(`/api/propertytypes/${route.params.id}/edit`, form)
        .then((response) => {
            router.push('/admin/propertytypes');
            toastr.success('Property type updated successfully!');
        })
        .catch((error) => {
            actions.setErrors(error.response.data.errors);
        })
...
const getPropertyType = () => {
    axios.get(`/api/propertytypes/${route.params.id}/edit`)
        .then(({ data }) => {
            console.log(data);
            form.id = data.id;
            form.name = data.name;
...
const handleFileChange = async (event) => {
    ...
    axios.post('/api/propertytypes/upload-image', formData)
        .then((response) => {
            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item">
                            <router-link to="/admin/propertytypes">Property Type</router-link>
                        </li>
